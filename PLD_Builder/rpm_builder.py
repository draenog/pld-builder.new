    ...
    if res:
        ...
        if not res:
            ...
            if not res:
                ...
                cmd = "cd rpm/SPECS; TMPDIR=%s nice -n %s rpmbuild -bb --define '_smp_mflags -j%d' --define '_specdir %s' --define '_sourcedir %s' %s %s" % \
                            (tmpdir, config.nice, max_jobs, packagedir, packagedir, rpmbuild_opt, b.spec)
